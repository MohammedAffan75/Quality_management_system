...
import { ArrowLeft, Layers, Edit2, Trash2, Plus, ChevronRight, ChevronDown, Box, Package, Menu, FileText, Download, Upload, Square, Folder, ClipboardCheck } from 'lucide-react';
...
const refreshDocuments = async () => {
  try {
    ...
    setDocuments(docs);
    console.log('Documents refreshed:', docs);
  } catch (error) {
...
// Fetch documents when component mounts AND when assembly is selected
// Fetch documents when component mounts AND when assembly is selected
useEffect(() => {
    ...
    try {
      ...
      docs.forEach((doc, index) => {
        console.log(`📄 Document ${index + 1}:`, {
          ...
          file_2d_name: doc.file_2d_name,
          file_3d_name: doc.file_3d_name
        });
      });
...
// NEW: Refresh documents when an assembly is selected
useEffect(() => {
  if (selectedNode && selectedNode.type === 'assembly' && !selectedNode.isDirectPart) {
    console.log('🔄 Assembly selected, refreshing documents for ID:', selectedNode.id);
    refreshDocuments();
  }
}, [selectedNode?.id]);
...
if (modalData.type === 'part' && !modalData.isDirectPart) {
  ...
  try {
    ...
    if (!modalData.editId) {
      if (modalData.type === 'part' && modalData.isDirectPart) {
        setSelectedNode({ ...newPart, isDirectPart: true });
      } else if (modalData.type === 'part' && !modalData.isDirectPart) {
        setSelectedNode({ ...newPart, isDirectPart: false });
      } else {
...
const handleCreateInspectionPlan = (node) => {
  ...
  // Find PDF document - check for file_2d_name or file_format === 'pdf'
  const pdfDocument = partDocuments.find(doc => {
    // Check if it's a 2D document (PDF)
    return doc.file_2d_name || 
           doc.file_format === 'pdf' || 
           (doc.file_format_2d && doc.file_format_2d.toLowerCase() === 'pdf') ||
...
const getTooltipContent = (buttonType) => {
  ...
  <button 
    ...
    <Layers size={14} />
            ...
            {isDirectPart && (
              <button onClick={(e) => { e.stopPropagation(); handleCreateInspectionPlan(node); }}
                style={{ background: '#e0e7ff', color: '#4f46e5', border: '1px solid #a5b4fc', borderRadius: '4px', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
...
const DocumentsSection = ({ assemblyId, isDirectPart = false, partId = null }) => {
  const assemblyDocuments = getAssemblyDocuments(assemblyId, isDirectPart, partId);
    ...
    return (
      <div style={{
        ...
        <div style={{ fontSize: '0.75rem', color: '#9ca3af' }}>
          {isDirectPart ? 'Part ID' : 'Assembly ID'}: {partId || assemblyId} • 
          Check if documents exist with this {isDirectPart ? 'part_id' : 'assembly_id'}
        </div>
      </div>
    );
  }
    ...
    <div style={{ marginBottom: '1.5rem' }}>
      <div style={{
        ...
        <h4 style={{ 
          ...
          <FileText size={16} />
          {isDirectPart ? 'Direct Part Documents' : 'Part Documents'} (ID: {partId || assemblyId})
        </h4>
      </div>
      
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        {assemblyDocuments.map((doc, idx) => (
...
{/* Enhanced Universal Documents Display - Works for Assembly, Subassembly, and Parts */}
    ...
    {selectedNode && (
                          <div style={{ marginBottom: '1.5rem' }}>
                            ...
                            {/* CORRECTED: Use proper filtering logic for each node type */}
                            {(() => {
                              let nodeDocuments = [];
                              
                              if (selectedNode.isDirectPart) {
                                // Direct Part: Filter by part_id
                                nodeDocuments = getAssemblyDocuments(null, true, selectedNode.id);
                              } else if (selectedNode.type === 'part') {
                                // Part: Filter by part_id  
                                nodeDocuments = getAssemblyDocuments(null, true, selectedNode.id);
                              } else {
                                // Assembly/Subassembly: Filter by assembly_id
                                nodeDocuments = getAssemblyDocuments(selectedNode.id, false, null);
                              }
                              
                              return nodeDocuments.length > 0;
                            })() ? (
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                {(() => {
                                  let nodeDocuments = [];
                                  ...
                                  return nodeDocuments.map((doc, idx) => (
                                    <React.Fragment key={doc.id || idx}>
                                      {/* 2D Document Card */}
                                      {doc.doc_type === '2D' && (
                                        <div style={{ 
