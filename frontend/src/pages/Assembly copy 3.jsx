...
const buildAssemblyTree = (assemblies, parentId = null) => {
  return assemblies
    ...
    .map(assembly => ({
      ...assembly,
      parts: buildAssemblyTree(assemblies, assembly.id)
    }));
};
...
useEffect(() => {
  const fetchAssemblies = async () => {
    ...
    try {
      const refreshedAssemblies = await fetchAllAssemblies();
      const currentProjectAssemblies = refreshedAssemblies.filter(assembly => 
        assembly.project_id === selectedProject.id || 
        assembly.project_id === parseInt(selectedProject.id)
      );
      
      // Build tree structure here
      const treeStructure = buildAssemblyTree(currentProjectAssemblies);
      setCurrentProject(prev => ({
        ...prev,
        assemblies: treeStructure
      }));
    } catch (error) {
...
const handleSave = async () => {
  ...
  const assemblyData = {
    ...
    parent_assembly_id: modalData.parent_assembly_id || null, // This will be set for sub-assemblies
    id: modalData.id,
...
// Update local state (without files since they're now in API)
// Replace this section (around line 445):
// Update local state (without files since they're now in API)
setCurrentProject(prev => ({
  ...prev,
          ...
          <div onClick={e => e.stopPropagation()} style={{ 
            ...
            <div style={{ 
              ...
              <h3 style={{ fontSize: '1.25rem', fontWeight: '600', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Layers size={20} />
{modalData.editId ? 'Edit' : modalData.parent_assembly_id ? 'Add Sub' : 'Add'} {modalData.type === 'part' ? (modalData.isDirectPart ? 'Direct Part' : 'Part') : 'Assembly'}           </h3>
            </div>
